...
import { renderStatusBarSvg, type StatusBarMode } from "./statusbar.ts";
...
export type PhoneFrameOptions = {
   ...
   */
  landscape?: boolean;
  /** Ekran görüntüsünün ekrana oturması: cover (kırp, varsayılan) / contain (sığdır). */
  fit?: "cover" | "contain";
...
export function phoneMockupHtml(opts: PhoneMockupOptions): string {
  ...
  const front = phoneFrameSvg({
    ...
    statusBarScrim: opts.statusBarScrim,
  });
...
export function tabletMockupHtml(opts: TabletMockupOptions): string {
  ...
  const front = `<div style="position:absolute;inset:0;transform:translateZ(0.6px);border-radius:${radius}px;
    ...
    <div style="position:absolute;inset:${inset}px;border-radius:${screenR}px;overflow:hidden;background:#000;">
      <img src="${escAttr(screenshotSrc)}" style="width:100%;height:100%;object-fit:${opts.fit === "contain" ? "contain" : "cover"};display:block;"/>
...
export function phoneFrameSvg(opts: PhoneFrameOptions): string {
  ...
  // cover = "slice" (kırparak doldur), contain = "meet" (kırpmadan sığdır, letterbox).
  const par = opts.fit === "contain" ? "xMidYMid meet" : "xMidYMid slice";
  const imageEl = opts.landscape
    ? `<image href="${safeSrc}" x="${cx - sh / 2}" y="${cy - sw / 2}" width="${sh}" height="${sw}"
           preserveAspectRatio="${par}" transform="rotate(-90 ${cx} ${cy})"/>`
    : `<image href="${safeSrc}" x="${sx}" y="${sy}" width="${sw}" height="${sh}"
           preserveAspectRatio="${par}"/>`;

  const statusBarSvg = renderStatusBarSvg(opts.statusBar ?? "none", opts.statusBarScrim ?? true);
